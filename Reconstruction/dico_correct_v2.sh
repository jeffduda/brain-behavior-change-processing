...
cat << EOF
USAGE: 
`basename $0` -p/-n [-options] <outfile_root> <rpsmap> <maskmap> Dicom_folder/ 
	or
`basename $0` -p/-n [-options] <outfile_root> <rpsmap> <maskmap> dcm1 dcm2 ... dcmN
	or
`basename $0` -p/-n [-options] -e <example_dicom> <outfile_root> <rpsmap> <maskmap> <EPIfile>
...
OPTIONS:
    ...
    -x  Extend voxel shiftmap outside mask (i.e. no --noextend option in Fugue)
...
while getopts "f:e:s:dumkhFSpnx" Option
do
  case $Option in
	s ) do_smooth=$OPTARG;;
	d ) de_oblique=1;;
...
if [ X$magfile != "X" ]; then
    echo "Coregistering B0map to EPI image(s)."
    ...
    flirt -ref $inputfile -in $rpsfile                         -init ${outfroot}.mat -o ${outfroot}_rpsmap_coreg -applyxfm      #  regrid to target epi

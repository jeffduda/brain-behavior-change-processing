...
while getopts "r:duFktemh" Option
do
  case $Option in
	d ) de_oblique=1;;
	u ) use_dcm2nii=1;;
...
cat << EOF
USAGE: `basename $0` [-duFtemkh] [-r Y/N] outfile dcmfile1 [dcmfile2 ... dcmfileN]
...
if [ $multi_volume = "1" -a $use_dcm2nii = "0" ]; then echo "ERROR: -m option requires -u option"; exit 1; fi
...
# --- (This is primarily to get the bvec and bval files for concatenated DTI runs) ---
    ...
    echo "Converting $ndcms multi-volume, multi-series Dicoms with dcm2nii..." | tee -a $logfile
    tmpdir=`mktemp -d`                              # make a temp working folder
    cp -f ${dcmfiles[*]} $tmpdir                    # need to operate on all dicoms in a single folder
    tmproot=`echo $dcmfroot | sed 's/\.//g'`       # get expected result filename from dcm2nii ('.' are removed!)
    dcm2nii -g N -p N -d N -e N -i N -v Y -f Y $tmpdir/* >> $logfile
    niftis=(`ls $tmpdir/*.nii`)
    n=${#niftis[@]}
    echo "Converted $n volumes. Concatenating them..."

...
cat << EOF
Usage:
       `basename $0` [-dukxmFS2h] outfile_root magnitude_dicom_folder/ phase_dicom_folder/ 
   or  `basename $0` [-dukxmFS2h] outfile_root mag_dcm1 mag_dcm2 ... mag_dcmN phase_dcm1 phase_dcm2 ... phase_dcmN
   
Mask options:
       `basename $0` [-dukxmFS2h] -s [-f BET_f] [-g BET_g]            
   or  `basename $0` [-dukxmFS2h] -T T1head.nii -B T1brain.nii 
   or  `basename $0` [-dukxmFS2h] -b brainmask.nii 
...
while getopts "T:B:b:f:g:sSdumkhFx2" Option
do
  case $Option in
	T ) t1whole=$OPTARG;;
	B ) t1brain=$OPTARG;;
...
if [ $do_skullstrip = "1" -a -n "$t1whole" ];   then echo "ERROR: You should not choose BOTH -s and -T"; exit 1; fi
if [ $do_skullstrip = "1" -a -n "$brainmask" ]; then echo "ERROR: You should not choose BOTH -s and -b"; exit 1; fi
if [ -n "$t1whole" -a -n "$brainmask" ];        then echo "ERROR: You should not choose BOTH -T and -b"; exit 1; fi
...
if [ $do_filesort -eq 1 ]; then
  echo "Sorting dicom files by series, echo and instance number."
  ...
  for (( i=0;   i<$ndcm;  i++ )); do
    ...
    if [ $i -eq 0 ]; then                              # find how many images from each series
        series1=$snum
    fi
    if [ $snum -eq $series1 ]; then
        let series1_count+=1
    else
        let series2_count+=1
    fi    
  ...
  if [ $is_siemens_fieldmap -eq 1 ]; then
    if [ $series1_count -eq $series2_count ]; then # dicoms were renamed so as to overwrite one of the magnitude volumes! 
        echo "ERROR: Siemens gre_fieldmapping sequence should make 2 magnitude and 1 phase image! Possible file renaming error!"
        exit 1
...
echo "  $cmd"
eval $cmd
if [ $? -ne 0 ]; then echo "ERROR: Prelude error - try not using the -u option to this script."; exit 1; fi # possibly "scl_slope" = 2 in Nifti header
